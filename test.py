...
GPIO.setup(ledB_Internet, GPIO.OUT)
...
def InitSys():
        ...
        print("Init Sys....")
        Mode_state = _Init
        GPIO.output(ledB_Internet, True)   
        GPIO.output(ledR_Error, True)   
        GPIO.output(ledG_Standby, True)
        #led 초기화
        GPIO.output(ledB_Internet, False)   
...
while True:
        ...
        if not CNT_num % 100 and not CNT_num==0:
            if not CNT_num % 600:
                Sum = tick_CNT*0.2
